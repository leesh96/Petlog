...
try {
    for($i = 0; $i < count($data); $i++) {
        $nick = $data[$i]['follower'];
        $sql = "select * from petsta_post where nickname='$nick' order by write_time DESC";
        $result = mysqli_query($link, $sql);
        if($result) {
            while($row=mysqli_fetch_array($result)) {
                array_push($post, 
            ...
            $json = json_encode(array("myfeed"=>$post), JSON_PRETTY_PRINT+JSON_UNESCAPED_UNICODE);
            echo $json;
        }
        if($stmt->execute())
        {
