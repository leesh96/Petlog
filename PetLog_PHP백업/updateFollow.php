    ...
    if( (($_SERVER['REQUEST_METHOD'] == 'POST') && isset($_POST['submit'])) || $android )
    {
        ...
        $isfollowed=$_POST['isfollowed'];
            ...
            try{
                ...
                $stmt = $con->prepare('UPDATE follow SET isfollow = (:isfollowed) WHERE user_nickname = (:mynick) and follower_nickname = (:usernick)');
                $stmt->bindParam(':isfollowed', $isfollowed);
                $stmt->bindParam(':mynick', $mynick);
                $stmt->bindParam(':usernick', $usernick);

                if($stmt->execute())
                {
                    $successMSG = "구독 상태 업데이트";
                }
                else
                {
                    $errMSG = "실패";
